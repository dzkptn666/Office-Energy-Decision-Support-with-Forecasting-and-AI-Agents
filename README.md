# Office-Energy-Decision-Support-with-Forecasting-and-AI-Agents
The following Jupyter Notebooks are provided for reproducing the experiments:

- `model_training.ipynb`: This notebook contains the training process for the electricity consumption forecasting model.

- `model_evaluation.ipynb`: This notebook evaluates the forecasting performance on seen and unseen buildings.

- `bertscore_evaluation.ipynb`: This notebook evaluates AI-generated energy-saving recommendations.
